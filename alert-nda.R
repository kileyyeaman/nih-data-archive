...
subjectDat <- subset(guidDat, select = -c(FIRSTNAME, MIDDLENAME, LASTNAME, SUBJECTHASMIDDLENAME, COB))
write.csv(subjectDat, '/Users/kileyyeaman/Desktop/NDAR Subject Info.csv', row.names = FALSE)
...
guidDat <- subset(guidDat, select = -c(child_dob, visit_date))
